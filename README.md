...
dropdown.*customSearch*        | <sub>Function</sub>          |                                             | If defined, filtering and ordering of suggestions is done via this function instead of built-in mechanisms. The function receives one argument: a list of options sourced from the whitelist. The function should return filtered and ordered list of suggestions. Respects `dropdown.includeSelectedTags` when sourcing the whitelist.
