    ...
    Tagify.prototype = {
        ...
        buildDataList: function () {
            ...
            for (i = this.settings.whitelist.length; i--;) {
                ...
                $(optionElm).text(this.settings.whitelist[i]);
                $('select', datalist).append(optionElm);
            }
        ...
        markTagByValue: function (value) {
            var idx = this.value.filter(function (item) {
        ...
        /**
         * return clean string to it's original value, replacing special characters representation like &#34; to '
         * to avoid broken html
         * @param {String} value
         * @returns {String}
         */
        desanitizeText: function (value) {
            return value.trim().split("&#39;").join("'").split('&#34;').join('"');
        },
        /**
        ...
        addTag: function (value) {
            ...
            result = value.split(this.settings.delimiters).filter(function (v) {
                return !!v
            }).map(function (v) {
                ...
                $('span', tagElm).text(that.desanitizeText(v) + ' ');
                that.DOM.scope.insertBefore(tagElm, that.DOM.input.parentNode);

                if (tagAllowed) {
                    that.value.push(v);
                    that.update();
                    that.trigger('add', {value: that.desanitizeText(v), index: that.value.length});
                }
        ...
        removeTag: function (idx, silent) {
            ...
            if (!silent) {
                ...
                this.trigger('remove', {value: this.desanitizeText(tagElm.textContent), index: idx});
            }
        },
