                ...
                */
                ...
                deleteBackspaceTimeout = setTimeout(() => {
                  var sel = document.getSelection(),
                      ...
                      prevElm = !deleteKeyTagDetected && sel.anchorNode.previousElementSibling; // fixes #384, where the first and only tag will not get removed with backspace

                  if (!isChromeAndroidBrowser() && currentValue.length >= lastInputValue.length && prevElm && !prevElm.hasAttribute('readonly')) {
                    if (prevElm.nodeName == 'BR') prevElm.remove();else {
      ...
      onInputDOMChange(m) {
        ...
        m.forEach(record => {
          ...
          record.addedNodes.forEach(addedNode => {
            if (addedNode) // fix chrome's placing '<div><br></div>' everytime ENTER key is pressed, and replace with just `<br'
        ...
        if (lastChild && lastChild.nodeValue == '') lastChild.remove();else if (lastChild && lastChild.nodeName == 'BR' && !m[0].removedNodes.length && isNodeTag.call(this, lastChild.previousSibling)) {
        ...
        rootNode.childNodes.forEach(node => {
          if (node.nodeType == 1) {
            ...
            if (node.tagName == 'BR' && (node.parentNode == that.DOM.input || node.parentNode.childNodes.length == 1)) {
              ...
              result += "\r\n";
              if (!node.children.length && node.textContent) result += node.textContent;
            }
