...
(function (global, factory) {
  ...
  Tagify.prototype = {
    ...
    getWhitelistItemByValue: function getWhitelistItemByValue(value) {
      ...
      try {
        for (var _iterator = _s.whitelist[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          item = _step.value;
          if (sameStr(typeof item == 'string' ? item : item.value, value, _s.dropdown.caseSensitive)) // normalize response
    ...
    parseMixTags: function parseMixTags(s) {
      ...
      s = s.split(mixTagsInterpolator[0]).map(function (s1, i) {
        ...
        if (s2.length > 1 && (!enforceWhitelist || _this9.isTagWhitelisted(tagData.value)) && !(!duplicates && _this9.isTagDuplicate(tagData.value))) {
          ...
          tagElm.classList.add(_this9.settings.classNames.tagNoAnimation);
    ...
    addTags: function addTags(tagsItems, clearInput) {
      ...
      tagsItems.forEach(function (tagData) {
        ...
        _s.transformTag.call(_this10, tagData);

        if (tagData.__isValid !== true) {
          if (skipInvalid) return;
          extend(tagElmParams, _this10.getInvalidTagParams(tagData, tagData.__isValid), {
            __preInvalidData: originalData
          }); // mark, for a brief moment, the tag THIS CURRENT tag is a duplcate of

          if (tagData.__isValid == _this10.TEXTS.duplicate) _this10.flashTag(_this10.getTagElmByValue(tagData.value));
        } /////////////////////////////////////////////////////
    ...
    addMixTags: function addMixTags(tagsItems) {
      ...
      setTimeout(function () {
        return tagElm.classList.add(_this11.settings.classNames.tagNoAnimation);
      }, 300);
      ...
      !createdFromDelimiters && setTimeout(function () {
        _this11.insertAfterTag(tagElm);

        _this11.placeCaretAfterTag(tagElm);
    ...
    reCheckInvalidTags: function reCheckInvalidTags() {
      ...
      [].forEach.call(tagElms, function (node) {
        var tagData = _this12.tagData(node),
    ...
    removeTags: function removeTags(tagElms, silent, tranDuration) {
      ...
      this.settings.hooks.beforeRemoveTag(tagsToRemove, {
        ...
        if (tranDuration && tranDuration > 10 && tagsToRemove.length == 1) animation.call(_this13, tagsToRemove[0]);else tagsToRemove.forEach(removeNode.bind(_this13)); // update state regardless of animation
        ...
        if (!silent) {
          tagsToRemove.forEach(function (tag) {
            ...
            var tagIdx = _this13.getTagIdx(tagData);

            if (tagIdx > -1) _this13.value.splice(tagIdx, 1);
          }); // that.removeValueById(tagData.__uid)

          _this13.update(); // update the original input with the current value
