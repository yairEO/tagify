...
(function (global, factory) {
  ...
  var dropdownMethods = {
      ...
      callbacks: {
        ...
        onClick: function onClick(e) {
          ...
          this.settings.hooks.suggestionClick(e, {
            ...
            if (listItemElm) _this4.dropdown.selectOption.call(_this4, listItemElm);
          })["catch"](function (err) {
            return err;
          });
        },
    ...
    selectOption: function selectOption(elm) {
      ...
      if (this.state.editing) {
        this.onEditTagDone(this.state.editing.scope, _objectSpread2(_objectSpread2(_objectSpread2({}, this.state.editing.scope.__tagifyTagData), {}, {
          value: tagData.value
    ...
    filterListItems: function filterListItems(value) {
      ...
      for (; i < whitelist.length; i++) {
        ...
        if (_sd.fuzzySearch) {
          ...
          whitelistItemValueIndex = _sd.accentedSearch ? unaccent(searchBy).indexOf(unaccent(niddle)) : searchBy.indexOf(niddle);
    ...
    createListHTML: function createListHTML(optionsArr) {
      ...
      return optionsArr.map(function (suggestion, idx) {
        ...
        var mapValueTo = _this7.settings.dropdown.mapValueTo,
        ...
        return _this7.settings.templates.dropdownItem.call(_this7, data);
      }).join("");
    }
  };
  ...
  var DEFAULTS = {
    ...
    mixMode: {
      insertAfterTag: "\xA0" // String/Node to inject after a tag has been added
  ...
  var events = {
    ...
    callbacks: {
            ...
            case 'Backspace':
              {
                ...
                setTimeout(function () {
                  var currentValue = decode(_this3.DOM.input.innerHTML); // fixes #384, where the first and only tag will not get removed with backspace
      ...
      onPaste: function onPaste(e) {
        ...
        if (this.settings.mode == 'mix') this.injectAtCaret(pastedData, window.getSelection().getRangeAt(0));else this.addTags(pastedData);
      },
  ...
  Tagify.prototype = {
      ...
      if (value) {
        ...
        if (this.settings.mode == 'mix') this.parseMixTags(value.trim());else {
          ...
          this.addTags(value).forEach(function (tag) {
            return tag && tag.classList.add(_this.settings.classNames.tagNoAnimation);
          });
        }
    ...
    placeCaretAfterTag: function placeCaretAfterTag(node) {
    ...
    editTag: function editTag(tagElm, opts) {
      ...
      editableElm.addEventListener('keydown', function (e) {
        return _CB.onEditTagkeydown.call(_this2, e, tagElm);
      });
      editableElm.focus();
    ...
    replaceTag: function replaceTag(tagElm, tagData) {
      ...
      if (tagData.__isValid && tagData.__isValid != true) extend(tagData, this.getInvalidTagParams(tagData, tagData.__isValid));
    ...
    updateValueByDOMTags: function updateValueByDOMTags() {
      ...
      [].forEach.call(this.getTagElms(), function (node) {
        if (node.classList.contains(_this3.settings.classNames.tagNotAllowed)) return;

        _this3.value.push(_this3.tagData(node));
      });
      this.update();
    },
    ...
    isTagDuplicate: function isTagDuplicate(value, caseSensitive) {
      ...
      duplications = this.value.reduce(function (acc, item) {
        return sameStr(_this4.trim("" + value), item.value, caseSensitive || _s.dropdown.caseSensitive) ? acc + 1 : acc;
      }, 0);
      return duplications;
    },
    getTagIndexByValue: function getTagIndexByValue(value) {
      ...
      this.getTagElms().forEach(function (tagElm, i) {
        if (sameStr(_this5.trim(tagElm.textContent), value, _this5.settings.dropdown.caseSensitive)) indices.push(i);
      });
      return indices;
    },
    ...
    flashTag: function flashTag(tagElm) {
      ...
      if (tagElm) {
        ...
        setTimeout(function () {
          tagElm.classList.remove(_this6.settings.classNames.tagFlash);
        }, 100);
      }
    },
    ...
    isTagWhitelisted: function isTagWhitelisted(v) {
      ...
      return this.settings.whitelist.some(function (item) {
        return typeof v == 'string' ? sameStr(_this7.trim(v), item.value || item) : sameStr(JSON.stringify(item), JSON.stringify(v));
      });
    },
    ...
    validateTag: function validateTag(tagData) {
      var value = this.trim(tagData.value),
          _s = this.settings; // check for empty value

      if (!tagData.value.trim()) return this.TEXTS.empty; // check if pattern should be used and if so, use it to test the value

      if (_s.pattern && _s.pattern instanceof RegExp && !_s.pattern.test(value)) return this.TEXTS.pattern; // if duplicates are not allowed and there is a duplicate

      if (!_s.duplicates && this.isTagDuplicate(value, this.state.editing)) return this.TEXTS.duplicate;
      if (this.isTagBlacklisted(value) || _s.enforceWhitelist && !this.isTagWhitelisted(value)) return this.TEXTS.notAllowed;
      return true;
    },
    getInvalidTagParams: function getInvalidTagParams(tagData, validation) {
      return {
        "aria-invalid": true,
    ...
    normalizeTags: function normalizeTags(tagsItems) {
        ...
        tagsItems = mapStringToCollection(tagsItems);
          ...
          tagsItems = (_ref2 = []).concat.apply(_ref2, _toConsumableArray(tagsItems.map(function (item) {
            return item.value ? mapStringToCollection(item.value).map(function (newItem) {
              return _objectSpread2(_objectSpread2({}, item), newItem);
      ...
      if (whitelistWithProps) {
        tagsItems.forEach(function (item) {
          ...
          var matchObj = _this8.getWhitelistItemByValue(item.value);
     ...
     */
    getWhitelistItemByValue: function getWhitelistItemByValue(value) {
      var _this9 = this;

      var result = this.settings.whitelist.find(function (item) {
        return sameStr(typeof item == 'string' ? item : item.value, value, _this9.settings.dropdown.caseSensitive);
      }); // normalize response
    ...
    addTags: function addTags(tagsItems, clearInput) {
      ...
      tagsItems.forEach(function (tagData) {
        ...
        _s.transformTag.call(_this11, tagData);

        if (tagData.__isValid !== true) {
          if (skipInvalid) return;
          extend(tagElmParams, _this11.getInvalidTagParams(tagData, tagData.__isValid), {
    ...
    addMixTags: function addMixTags(tagsItems) {
      ...
      this.update(); // fixes a firefox bug where if the last child of the input is a tag and not a text, the input cannot get focus (by Tab key)

      !createdFromDelimiters && setTimeout(function () {
        _this12.insertAfterTag(tagElm);

        _this12.placeCaretAfterTag(tagElm);
    ...
    removeTags: function removeTags(tagElms, silent, tranDuration) {
      ...
      tagsToRemove = tagElms.reduce(function (elms, tagElm) {
        ...
        if (tagElm) // because the DOM node might be removed by async animation, the state will be updated while
          ...
          elms.push({
            node: tagElm,
            idx: _this14.getTagIdx(_this14.tagData(tagElm)),
